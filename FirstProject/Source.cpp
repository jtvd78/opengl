...
int gen_knob() {
	const int rows = 12;
	const int cols = 30;
	const int number = 9 * (rows - 1) * cols * 2;
	num = number;

	float verts[number];
	fill_array(verts, rows, cols);
...
int main() {
	...
	Shader shader("color.vert", "passthrough.frag");
...
//	glEnableVertexAttribArray(1);
	...
	//Create texture
	unsigned int texture1 = load_texture("container.jpg");
	unsigned int texture2 = load_texture("awesomeface.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm::mat4 mod;
		mod = glm::translate(mod, glm::vec3(1.0f, 1.0f, 1.0f));
		mod = glm::rotate(mod, glm::radians(20.0f) + (float)glfwGetTime(), glm::vec3(0.0f, 1.0f, 0.0f));
		mod = glm::scale(mod, glm::vec3(0.2f, 0.2f, 0.2f));
		shader.setMat4("model", mod);

		glBindVertexArray(knob);
